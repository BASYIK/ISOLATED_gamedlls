...
void LoadSpreadTable( char *filename )
{
	...
	int length;
	char *pFile;
	char *aFile = pFile = (char*)g_engfuncs.pfnLoadFileForMe( filename, &length );
	...
	{		
		...
		gSpreadTable[gSCounter].pri_speed = 150;
		gSpreadTable[gSCounter].sec_speed = 130;
...
BEGIN_DATADESC(CBaseSpreadWeapon)
	DEFINE_FIELD(  m_flSpreadTime, FIELD_FLOAT ),
	DEFINE_FIELD(  m_flLastShotTime, FIELD_TIME ),
	DEFINE_FIELD(  m_flLastSpreadPower, FIELD_FLOAT ),
END_DATADESC()
...
BEGIN_DATADESC(CBaseToggleWeapon)
DEFINE_FIELD( m_iWeaponMode, FIELD_INTEGER),
END_DATADESC();
