...
void CVAL::Precache( void )
{
	...
	PRECACHE_SOUND ("weapons/val_fire.wav");
...
{
	InitToggling();
	return DefaultDeploy("models/v_val.mdl", "models/p_val.mdl", VAL_DRAW, "mp5", UseDecrement());
}
...
void CVAL::Attack1()
{
	...
	SendWeaponAnim(VAL_SHOOT);
	EMIT_SOUND_DYN(ENT(m_pPlayer->pev), CHAN_WEAPON, "weapons/val_fire.wav", RANDOM_FLOAT(0.92, 1.0), ATTN_NORM, 0, 98 + RANDOM_LONG(0, 3));
