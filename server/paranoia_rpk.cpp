...
//	return FALSE;
	...
	InitToggling();
	return DefaultDeploy( "models/v_rpk.mdl", "models/p_rpk.mdl", RPK_DRAW, "mp5", UseDecrement() );
}
...
void CRPK::Attack1()
{
	...
	m_pPlayer->FireBulletsPlayer( 1, vecSrc, vecAiming, vecSpread, 8192, BULLET_PLAYER_RPK, 2, 0, m_pPlayer->pev, m_pPlayer->random_seed );

	SendWeaponAnim(RPK_SHOOT_A);
	EMIT_SOUND_DYN(ENT(m_pPlayer->pev), CHAN_WEAPON, "weapons/rpk_fire1.wav", RANDOM_FLOAT(0.92, 1.0), ATTN_NORM, 0, 98 + RANDOM_LONG(0, 3));
...
int CRPK::ChangeModeTo1()
{
	...
	m_pPlayer->m_flNextAttack = gpGlobals->time + 0.57;
	m_flNextSecondaryAttack = m_flNextPrimaryAttack = m_pPlayer->m_flNextAttack;
	m_flTimeWeaponIdle = gpGlobals->time + 0.57 + 0.5;
	return 1;
}
