...
{
	DECLARE_CLASS(CMilitary, CTalkMonster);
public:
	CUSTOM_SCHEDULES;
	DECLARE_DATADESC();
	void Spawn( void );
...
//	void PrescheduleThink ( void );
	...
	virtual int	ObjectCaps( void ) { if (pev->deadflag == DEAD_NO) return CTalkMonster :: ObjectCaps() | FCAP_IMPULSE_USE; else return CTalkMonster::ObjectCaps(); }
...
//	DEFINE_FIELD( CMilitary, m_iDeadAmmo, FIELD_INTEGER ),
END_DATADESC();
...
void CMilitary :: GibMonster ( void )
{
	...
	if (GetBodygroup(MIL_GUN_GROUP) != MIL_GUN_NONE && !(pev->spawnflags & SF_MONSTER_NO_WPN_DROP))
	{
		...
		CBaseEntity *pGun = DropItem( "weapon_ak74", vecGunPos, vecGunAngles );
...
/*		case HGRUNT_AE_GREN_LAUNCH:
		...
		if ( RANDOM_LONG(0,1) )
		{
		EMIT_SOUND( ENT(pev), CHAN_WEAPON, "weapons/ak74_fire1.wav", 1, ATTN_NORM );			
		}
		{
		EMIT_SOUND( ENT(pev), CHAN_WEAPON, "weapons/ak74_fire2.wav", 1, ATTN_NORM );
	    }
			}
			else
			{
...
// can grenade launch
			...
			else if ((HasWeapon(MIL_GRENADES ) && HasConditions ( bits_COND_CAN_RANGE_ATTACK2 )))
			{
...
{
	DECLARE_CLASS(CSpetsnaz, CMilitary);
public:
	DECLARE_DATADESC();
	void Spawn( void );
	void Precache( void );
...
BEGIN_DATADESC(CSpetsnaz)
DEFINE_FIELD( m_iHasHeadShield, FIELD_INTEGER),
DEFINE_FIELD( m_iHasGrenades, FIELD_INTEGER),
END_DATADESC();
...
//	ALERT(at_console, "*** time %f\n", gpGlobals->time);
	...
	{
		m_fNextRadioNoise = gpGlobals->time + RANDOM_FLOAT( 20, 60 );
		SENTENCEG_PlayRndSz( ENT(pev), "AL_RADIONOISE", 0.2, ATTN_STATIC, 0, m_voicePitch);
	}
	CMilitary::RunAI();
}
...
#define GRU_HEAD_GROUP	1
#define GRU_HEAD_SHIELD	6

#define GRU_GUN_GROUP		2
#define GRU_WEAPON_AK74	0
#define GRU_WEAPON_EMPTY	1

class CSpetsnazGRU : public CSpetsnaz
{
public:
	void Spawn( void );
	void Precache( void );
...
LINK_ENTITY_TO_CLASS( monster_human_gru, CSpetsnazGRU );

void CSpetsnazGRU :: Spawn()
{
	Precache( );
	...
	if (pev->model)
		...
		SET_MODEL(ENT(pev), "models/kolpakov/soldier_gru.mdl");
	UTIL_SetSize(pev, VEC_HUMAN_HULL_MIN, VEC_HUMAN_HULL_MAX);
	...
	int head = pev->body;
	pev->body = 0;
	SetBodygroup( GRU_HEAD_GROUP, head );
	m_iHasHeadShield = (head == GRU_HEAD_SHIELD) ? 1 : 0;
	m_iNoGasDamage = 0;
	
	MonsterInit();
	SetUse(&CSpetsnazGRU :: FollowerUse );
}

//=========================================================
// Precache - precaches all resources this monster needs
//=========================================================
void CSpetsnazGRU :: Precache()
{
	if (pev->model)
		PRECACHE_MODEL((char*)STRING(pev->model)); //LRC
	else
		PRECACHE_MODEL("models/kolpakov/soldier_gru.mdl");
...
BOOL CSpetsnazGRU :: CheckRangeAttack1 ( float flDot, float flDist )
{
	return CMilitary :: CheckRangeAttack1 ( flDot, flDist );
}
...
void CSpetsnazGRU :: GibMonster ( void )
{
	Vector	vecGunPos;
	Vector	vecGunAngles;

	if (GetBodygroup(GRU_GUN_GROUP) != GRU_WEAPON_EMPTY && !(pev->spawnflags & SF_MONSTER_NO_WPN_DROP))
	{
		GetAttachment( 0, vecGunPos, vecGunAngles );
...
// that occur when tagged animation frames are played.
//=========================================================
void CSpetsnazGRU :: HandleAnimEvent( MonsterEvent_t *pEvent )
{
	Vector	vecShootDir;
	Vector	vecShootOrigin;
		...
		case MIL_AE_DROP_GUN:
			{
				...
				GetAttachment( 0, vecGunPos, vecGunAngles );
				// switch to body group with no gun.
				SetBodygroup( GRU_GUN_GROUP, GRU_WEAPON_EMPTY );
				DropItem( "weapon_ak74", vecGunPos, vecGunAngles );
...
//	}
	...
	if (m_cAmmoLoaded > 0)
	{
		...
		Vector	vecShellVelocity = gpGlobals->v_right * RANDOM_FLOAT(40,90) + gpGlobals->v_up * RANDOM_FLOAT(75,200) + gpGlobals->v_forward * RANDOM_FLOAT(-40, 40);
		EjectBrass ( vecBrassPos, vecShellVelocity, pev->angles.y, m_iBrassShell, TE_BOUNCE_SHELL); 
		FireBullets(1, vecShootOrigin, vecShootDir, VECTOR_CONE_5DEGREES, 2048, BULLET_PLAYER_AK47 );
...
// Wargon: Дефолтный TASK_DIE оверрайден для имитации действия энтити player_loadsaved. (1.1)
void CSpetsnazGRU :: RunTask ( Task_t *pTask )
{
}
...
class CGRURepel : public CBaseMonster
{
public:
	void Spawn( void );
	void Precache( void );
...
LINK_ENTITY_TO_CLASS( monster_gru_repel, CGRURepel );

void CGRURepel::Spawn( void )
{
	Precache( );
	pev->solid = SOLID_NOT;

	SetUse(&CGRURepel:: RepelUse );
}

void CGRURepel::Precache( void )
{
	UTIL_PrecacheOther( "monster_human_gru" );
	m_iSpriteTexture = PRECACHE_MODEL( "sprites/rope.spr" );
}

void CGRURepel::RepelUse ( CBaseEntity *pActivator, CBaseEntity *pCaller, USE_TYPE useType, float value )
{
	TraceResult tr;
	UTIL_TraceLine( pev->origin, pev->origin + Vector( 0, 0, -4096.0), dont_ignore_monsters, ENT(pev), &tr);
	/*
	...
	*/
	...
	CBaseEntity *pEntity = Create( "monster_human_gru", pev->origin, pev->angles );
	CBaseMonster *pGrunt = pEntity->MyMonsterPointer( );
	pGrunt->pev->movetype = MOVETYPE_FLY;
	pGrunt->pev->velocity = Vector( 0, 0, RANDOM_FLOAT( -196, -128 ) );
	pGrunt->SetActivity( ACT_GLIDE );
