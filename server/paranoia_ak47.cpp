...
void CAK47::Precache( void )
{
	...
	PRECACHE_SOUND ("weapons/akm_fp.wav");
...
{
	InitToggling();
	return DefaultDeploy("models/v_ak74.mdl", "models/p_ak74.mdl", AK47_DRAW, "mp5", UseDecrement());
}
...
void CAK47::Attack1()
{
	...
	m_pPlayer->FireBulletsPlayer( 1, vecSrc, vecAiming, vecSpread, 8192, BULLET_PLAYER_AK47, 2, 0, m_pPlayer->pev, m_pPlayer->random_seed );

	SendWeaponAnim(AK47_SHOOT_A);
	EMIT_SOUND_DYN(ENT(m_pPlayer->pev), CHAN_WEAPON, "weapons/akm_fp.wav", RANDOM_FLOAT(0.92, 1.0), ATTN_NORM, 0, 98 + RANDOM_LONG(0, 3));
